...
import bookProtocolDefinition from "./blackgirl";
...
export default function Home() {
  ...
  const configuration = async () => {
    ...
    const { protocol } = await web5.dwn.protocols.configure({
      message: {
        definition: bookProtocolDefinition
      },
    });
    ...
    console.log("PROTOCOL DEFINITION : ", definition);
      ...
      const { record: postRecord, status: createStatus } = await web5.dwn.records.create({
        ...
        message: {
          protocol: "https://sollertia.com/books",
          protocolPath: 'label',
          recipient: NiluDid,
          schema: "https://sollertia.com/books/label",
